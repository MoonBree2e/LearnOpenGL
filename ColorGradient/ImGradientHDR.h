...
enum class ImGradientMarkerType
{
	Color,
	Alpha,
	Unknown,
...
struct ImGradientTemporaryState
{
	ImGradientMarkerType selectedMarkerType = ImGradientMarkerType::Unknown;
	int selectedIndex = -1;

	ImGradientMarkerType draggingMarkerType = ImGradientMarkerType::Unknown;
	int draggingIndex = -1;
};

bool ImGradient(int32_t gradientID, ImGradientState& state, ImGradientTemporaryState& temporaryState, bool isMarkerShown = true);
