...
namespace
{
	...
	ImU32 GetMarkerColor(const ImGradientState::ColorMarker& marker)
	{
		const auto c = marker.Color;
		return ImGui::ColorConvertFloat4ToU32({ c[0], c[1], c[2], 1.0f });
	}

	ImU32 GetMarkerColor(const ImGradientState::AlphaMarker& marker)
	{
	...
	UpdateMarkerResult UpdateMarker(
		...
		int& markerCount,
		ImGradientTemporaryState& temporaryState,
		ImGradientMarkerType markerType,
		const char* keyStr,
		ImVec2 originPos,
		float width,
		float markerWidth,
		...
		MarkerDirection markerDir)
	{
		...
		for (int i = 0; i < markerCount; i++)
		{
			...
			if (!ImGui::IsMouseDown(0))
			{
				...
				temporaryState.draggingMarkerType = ImGradientMarkerType::Unknown;
			}
...
ImGradientState::ColorMarker* ImGradientState::GetColorMarker(int32_t index)
{
	if (index < 0 ||
		index >= ColorCount)
	{
...
ImGradientState::AlphaMarker* ImGradientState::GetAlphaMarker(int32_t index)
{
	if (index < 0 ||
		index >= AlphaCount)
	{
...
bool ImGradientState::AddColorMarker(float x, std::array<float, 3> color, float intensity)
{
...
bool ImGradientState::AddAlphaMarker(float x, float alpha)
{
...
bool ImGradientState::RemoveColorMarker(int32_t index)
{
...
bool ImGradientState::RemoveAlphaMarker(int32_t index)
{
...
std::array<float, 4> ImGradientState::GetCombinedColor(float x) const
{
	const auto c = GetColorAndIntensity(x);
	return std::array<float, 4>{c[0] * c[3], c[1] * c[3], c[2] * c[3], GetAlpha(x)};
}

std::array<float, 4> ImGradientState::GetColorAndIntensity(float x) const
{
	if (ColorCount == 0)
	{
...
float ImGradientState::GetAlpha(float x) const
{
	if (AlphaCount == 0)
	{
		return 1.0f;
...
bool ImGradient(int32_t gradientID, ImGradientState& state, ImGradientTemporaryState& temporaryState, bool isMarkerShown)
{
	bool changed = false;
	...
	if (isMarkerShown)
	{
		const auto resultAlpha = UpdateMarker(state.Alphas, state.AlphaCount, temporaryState, ImGradientMarkerType::Alpha, "a", originPos, width, markerWidth, markerHeight, MarkerDirection::ToLower);

		changed |= resultAlpha.isChanged;

		if (temporaryState.draggingMarkerType == ImGradientMarkerType::Alpha)
		{
		...
		if (ImGui::IsItemHovered())
		{
			...
			if (!resultAlpha.isHovered && state.AlphaCount < (int)state.Alphas.size())
			{
				DrawMarker(
					{ originPos.x + x - 5, originPos.y + markerHeight },
					{ originPos.x + x + 5, originPos.y + 0 },
	...
	if (isMarkerShown)
	{
		...
		const auto resultColor = UpdateMarker(state.Colors, state.ColorCount, temporaryState, ImGradientMarkerType::Color, "c", originPos, width, markerWidth, markerHeight, MarkerDirection::ToUpper);

		changed |= resultColor.isChanged;

		if (temporaryState.draggingMarkerType == ImGradientMarkerType::Color)
		{
		...
		if (ImGui::IsItemHovered())
		{
			...
			if (!resultColor.isHovered && state.ColorCount < (int)state.Colors.size())
			{
				DrawMarker(
					{ originPos.x + x - 5, originPos.y + 0 },
					{ originPos.x + x + 5, originPos.y + markerHeight },
